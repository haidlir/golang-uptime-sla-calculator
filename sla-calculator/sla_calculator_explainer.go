...
func (u *UptimeSLACalculator) ExplainBakti1Uptime() []Bakti1UptimeChronology {
	...
	for i := 0; i < len(countedVals); i++ {
		...
		if status == BaktiLinkFailure {
			duration := chronology.EndTimestamps - chronology.StartTimestamps
			chronology.LinkFailureDuration = duration
			chronology.RestitutionDuration = duration
		} else {
		...
		if chronologies[i].Status == BaktiLinkFailure {
			...
			if isLinkFailure {
				...
				for j := iEnd; tolerance > 0 && j >= iStart; j-- {
					restitution := chronologies[j].RestitutionDuration - tolerance
					if (restitution) < 0 {
						tolerance -= chronologies[j].RestitutionDuration
